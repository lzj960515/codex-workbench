...
Put the actual logic (the bit that's answering the question) in a single `<script>` block written as a small, pure module that could be lifted out and dropped into the real codebase later. The page around it is throwaway; this module preserves the model being evaluated for later implementation review.
...
Once the prototype has answered its question, capture the answer, then capture the prototype the way the [SKILL](SKILL.md) describes. Preserve the reducer / machine / function set as evidence for the decision, and keep the self-contained HTML file re-runnable. Record feedback-dependent choices as pending until the user answers. When implementation is in scope, review and adapt the logic at its real owner with the normal implementation and verification workflow.

## Anti-patterns

- **Verify the question directly.** Run the relevant walkthroughs and state transitions; add only the small checks that make their results trustworthy.
